...
```{r}
test = c()
for (i in seq_along(head(all_text, 20))) {
  # print(all_text[i])
  test <- c(test, strsplit(all_text[i], " "))
  ################### TODO #####################
  # remove unnessary white spaces
  # word stemming
  # maybe first apply all the functions, then save into clean character vector?
...
tmp <- left_join(x = df_subset, y = word_freq, by = "word")
ggplot(tmp, aes(x = x, y = y, label = word, size = n)) +
  geom_text_repel() + theme_void()
