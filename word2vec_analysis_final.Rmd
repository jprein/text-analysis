...
```{r load_data}
# READ IN THE DATA. we want a list with again list entries. each list entry is one paper (e.g. data_list[[1]])
...
# for (i in seq_along(file_list)) {
#    data_list[[i]] <- pdf_data(file_list[i]) %>% 
#     readr::read_lines()
# }
```
...
### dimensionality reduction maintains the relative relationships between the vectors. so it is different!!
