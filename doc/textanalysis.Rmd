...
file_list <- list.files(here("raw-data/txts"), pattern = "*.txt")
data_list <- vector("list", "length" = length(file_list))

### ERROR! 
# for every txt file, read in the text and save it in data list
for (i in 10) {
  # print(file_list[i])
  # data_list[[i]] <- read.delim(here("raw-data/txts", file_list[i]))
  data_list[[i]] <- read_tsv(here("raw-data/txts", file_list[i]), locale = readr::locale(encoding = "utf8"))
}

# sed 's/\\0//g' "/Users/julia_prein/Work/experiments/soc_cog_textanalysis/raw-data/txts/adrian2007mothers.txt" > mycsv.txt
# fread("sed 's/\\0//g' /Users/julia_prein/Work/experiments/soc_cog_textanalysis/raw-data/txts/adrian2007mothers.txt")

cleanFiles<-function(file,newfile){
  writeLines(iconv(readLines(file,skipNul = TRUE)),newfile)
}

cleanFiles("/Users/julia_prein/Work/experiments/soc_cog_textanalysis/raw-data/txts/bowman2017action.txt", "/Users/julia_prein/Work/experiments/soc_cog_textanalysis/raw-data/txts/bowman2017action_1.txt")
```
