...
```{r}
library(reticulate)
# Error in if (forge) "conda-forge" : 
#   argument is not interpretable as logical
conda_install("pdfminer", "os", "io")
```
...
# install SciPy
conda_install("r-reticulate", "pdfminer")

# import SciPy (it will be automatically discovered in "r-reticulate")

# Error in py_module_import(module, convert = convert) : 
#   ModuleNotFoundError: No module named 'pdfminer'
pdfminer <- import("pdfminer")
```
...
# install SciPy
virtualenv_install("r-reticulate", "pdfminer")

# import SciPy (it will be automatically discovered in "r-reticulate")
# ...... 
# Successfully installed pdfminer-20191125 pycryptodome-3.9.9
# Error in py_module_import(module, convert = convert) : ModuleNotFoundError: No module named 'pdfminer'
pdfminer <- import("pdfminer")
```


```{python}
# Python 3.8.3 (/Users/julia_prein/Library/r-miniconda/bin/python)
# Reticulate 1.16 REPL -- A Python interpreter in R.
# SyntaxError: invalid syntax (<string>, line 1)
conda activate pdfminer
```

```{r}
library(reticulate) # for running python in R 

# Error: could not find a Python environment for /Users/julia_prein/Library/r-miniconda/bin/python
py_install("pdfminer") # works

# py_install("os") # doesn't work
# py_install("io") # doesn't work
```
...
# needs main to function properly. but in main, I can't access r.file_list()
# if __name__ == '__main__':
#       
#     # läuft nicht, weil os nicht installiert  
#     # files = [f for f in os.listdir('.') if os.path.isfile(f)]
#     # files = filter(lambda f: f.endswith(('.pdf','.PDF')), files)
#     
#     text_file = open("test.txt", "w")
#     n = text_file.write(convert_pdf_to_txt("/Users/julia_prein/Work/experiments/soc_cog_textanalysis/raw-data/adrián2007mothers.pdf"))
#     text_file.close()
```
...
```{r}
# läuft theoretisch (aber fehler wenn import im .py)
# source_python("pdfconverter.py") # läuft nicht mit import in .py
# py_run_file("pdfconverter.py")
```

```{python}
print(r.file_list) # hier gehts (oben in der main nicht)
```
