...
run_python_file("test_code.py")
source_python("test_code.py")
```
...
# remove .pdf (or any extension) at the end of the file names (in general, basename also removes the path leading to a file)
file_list <- sub(pattern = "(.*)\\..*$", replacement = "\\1", basename(file_list))
...
# write pdfs into txt files
# for every pdf file, read in the text and save it in data list
for (i in seq_along(file_list)) {
  # for progress, print name of file 
  # (if there is an error, it's with the file name that was printed last)
  print(file_list[i])
  # use our python function to read in pdfs and save output into temporary variable
  tmp <- convert_pdf_to_txt(here("raw-data/pdfs", paste(file_list[i], ".pdf", sep = "")))
  # write output into txt file located in the folder raw-data/txts
  write.table(tmp, file = here("raw-data/txts", paste(file_list[i], ".txt", sep = "")), col.names = FALSE, row.names = FALSE, sep = "\n")
}
...
#####
# Error in py_ref_to_r(x) : Embedded NUL in string.
# - corrupted file? 
# - too large?
# - weird characters?
#####
...
# WORKS! DO IT ONCE; CONVERT PDFs TO TXT FILES! 
...
# read directly in, don't save as .txt
# for every pdf file, read in the text and save it in data list
for (i in seq_along(file_list)) {
  print(file_list[i])
   data_list[[i]] <- convert_pdf_to_txt(here("raw-data/pdfs", file_list[i])) %>% 
    readr::read_lines()
}
